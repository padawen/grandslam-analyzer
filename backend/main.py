...
# Global async client for reusing connections
...
async def get_tournaments_list(
    ...
    try:
        ...
        # remove duplicate dicts just in case, but IDs are unique
...
async def get_divisions(
    ...
    try:
        ...
        # Sort to keep ATP first if both present
...
async def get_categories(
    ...
    try:
        ...
        # Default to grand_slam if nothing found yet
        if not categories:
            categories = ["grand_slam"]
        return categories
    except Exception as e:
        return ["grand_slam"]  # Graceful fallback
